...
export class PartyService {
  ...
  async updatePartyType(updatePartyDto: UpdatePartyDto, party_id: string) {
    return await this.prisma.party.update({
      ...
      data: {
        date_time:
          updatePartyDto.date_time !== undefined
            ? new Date(updatePartyDto.date_time)
            : updatePartyDto.date_time,
  ...
  async readParticipantcount(party_id: string) {
    ...
    const current_participant_count = await this.prisma.participant.count({
      where: { party_id },
    });
