...
export class GuestService {
  ...
  private toPartyModel(dto: PartyInfoGuestDto): Party {
    return {
      ...
      party_state: true,
      mid_lat: dto.mid_lat
        ? new Prisma.Decimal(dto.mid_lat)
        : new Prisma.Decimal(0),
      mid_lng: dto.mid_lng
        ? new Prisma.Decimal(dto.mid_lng)
        : new Prisma.Decimal(0),
  ...
  // --- 2) Participant 변환 ---
  private async toParticipantModel(
    dto: ParticipantGuestDto,
    party_id: string,
  ): Promise<Participant> {
    let lat = null;
    let lng = null;
  ...
  private toCourseModel(dto: CourseGuestDto, party_id: string): Course {
    return {
      ...
      place_lat: dto.place_lat
        ? new Prisma.Decimal(dto.place_lat)
        : new Prisma.Decimal(0),
      place_lng: dto.place_lng
        ? new Prisma.Decimal(dto.place_lng)
        : new Prisma.Decimal(0),
  ...
  async guestParty(dto: GuestDto) {
    ...
    const participants = await Promise.all(
      dto.participants.map((p) => this.toParticipantModel(p, party.party_id)),
    );
    ...
    const arr = await this.kakaoService.findAICoursePlaces(
      courses,
      midpoint.lat,
      midpoint.lng,
    );
      ...
      (async () => {
        ...
        const resolvedPlaces = await Promise.all(
          arr.distance.map(async (l) => ({
            ...
            imageUrl: await this.commonSerivce.getPlaceImageUrl(
              l.place.place_url,
            ),
          })),
        );
  ...
  async guestResult(dto: GuestDto) {
    ...
    const courses = await Promise.all(
      dto.courses.map((c) => this.toCourseModel(c, party.party_id)),
    );
    const members = await Promise.all(
      participants.map(async (p) => {
        ...
        const fastest = route.plan.itineraries.reduce((a, b) =>
      ...
      party: {
        ...
        courses: courses.map((c) => ({
          ...
          places: {
            ...
            imageUrl:
              dto.courses.find((course) => course.course_id === c.course_id)
                ?.imageUrl || '',
          },
        })),
      },
