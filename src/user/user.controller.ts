...
export class UserController {
  ...
  @HttpCode(HttpStatus.OK)
  @Get('find-id')
  @ApiOperation({ summary: 'ID 찾기', description: '가입된 이메일로 ID 조회' })
  @ApiQuery({ 
    name: 'email', 
    type: 'string',
  ...
  @Get('user-info')
  @ApiBearerAuth()
  @ApiOperation({ summary: '사용자 정보 조회', description: 'JWT로 로그인된 사용자 정보 반환' })
  @ApiResponse({ 
    status: 404, 
    description: '사용자 정보 없음', 
    schema: { 
  ...
  @ApiOperation({ 
  	summary: '사용자 정보 수정', 
  	description: '이메일 인증 완료 사용자만 수정 가능. 이메일 변경하면 이메일 인증 필수' 
  	})
  @ApiResponse({ 
    status: 408, 
  ...
  @ApiOperation({ summary: '비밀번호 재설정 요청', description: '입력된 이메일로 재설정 메일 발송' })
  @ApiResponse({ 
    status: 404, 
    description: '사용자 정보 없음', 
    schema: { 
  ...
  @ApiBearerAuth()
  @ApiOperation({ summary: '비밀번호 변경', description: '로그인된 상태에서 현재 비밀번호 검증 후 새 비밀번호로 변경' })
  @ApiBody({ 
  ...
  @Delete('delete-user')
  @ApiBearerAuth()
  @ApiOperation({ summary: '회원 탈퇴', description: '해당 계정 및 관련 데이터 삭제' })
  @ApiResponse({ 
    status: 404, 
    description: '사용자 정보 없음', 
    schema: { 
